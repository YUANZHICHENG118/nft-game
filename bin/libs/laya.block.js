...
// erc1155代币id 1-18分别对应:采矿车1-6;翻斗车7-12;挖掘机13-18 颜色顺序[白 绿 蓝 紫 粉 橙 ]
    ...
    const blockUrl="https://api.thegraph.com/subgraphs/name/blocklytics/ethereum-blocks"

    const apiUrl = dev ? "" : "https://api.cmblk.com";
    //交易所地址
    const exchangeUrl = "/market/#/swap";
    //交易NFT所地址
    ...
    class LayaBlock {
        ...
        static exUrl =()=>{
              return exchangeUrl+"?inputCurrency=ETH&outputCurrency="+this.erc20TokenAddress
        }
        ...
        static  getUserMine = async () => {
            ...
            const reward = ethAmount*parseFloat(ethPrice).toFixed(2)
         ...
         */
        static market = async () => {
           const data= await this.tokenData("0x1f9840a85d5af5bf1d1762f925bdaddc4201f984")

            let symbol = this.erc20Token.symbol;
            let market = {price: parseFloat(data&&data.priceUSD).toFixed(2), rate: data&&data.priceChangeUSD>0?"+"+ parseFloat(data&&data.priceChangeUSD).toFixed(2):parseFloat(data&&data.priceChangeUSD).toFixed(2), symbol: symbol, priceSymbol: '$', high: 10, low: 5, vol: parseFloat(data&&data.oneDayVolumeUSD).toFixed(2)+"$"};
            return new Promise(function (resolve, reject) {
                resolve(market)
            });
        }
        ...
        static  getPlayDetail = async (gameId, address) => {
          ...
          static getBlockFromTimestamp= async(timestamp)=> {
...
`
              ...
              return new Promise(function (resolve, reject) {
                  req.once(Laya.Event.COMPLETE, this, (result) => {
                      let number=result&&result.data&&result.data.blocks[0].number
        ...
        static TOKEN_DATA=(tokenAddress, block)=>{
            const TokenFields = `
...
`
  ...
  `
        ...
        static get2DayPercentChange = (valueNow, value24HoursAgo, value48HoursAgo) => {
        ...
        static  tokenData = async (tokenAddress, block) => {
           let ethPrice=await this.getETHPrice();
            let ethPriceOld=1793.751789258666922712958739038362;
            const utcCurrentTime = dayjs()
            const utcOneDayBack = utcCurrentTime.subtract(1, 'day').unix()
            const utcTwoDaysBack = utcCurrentTime.subtract(2, 'day').unix()
            let oneDayBlock = await this.getBlockFromTimestamp(utcOneDayBack)
            let twoDayBlock = await this.getBlockFromTimestamp(utcTwoDaysBack)
            let data = {}
            let oneDayData = {}
            let twoDayData = {}

            let result=await this.TOKEN_DATA(tokenAddress)
            data=result&&result.data&&result.data.tokens[0]

            // get results from 24 hours in past
            let oneDayResult=await this.TOKEN_DATA(tokenAddress,oneDayBlock)
            oneDayData = oneDayResult.data.tokens[0]

            // get results from 48 hours in past
            let twoDayResult = await this.TOKEN_DATA(tokenAddress,twoDayBlock)
            ...
            const [oneDayVolumeUSD, volumeChangeUSD] = this.get2DayPercentChange(
                data.tradeVolumeUSD,
                oneDayData&&oneDayData.tradeVolumeUSD || 0,
                twoDayData&&twoDayData.tradeVolumeUSD || 0
            )

            // calculate percentage changes and daily changes
            const [oneDayVolumeUT, volumeChangeUT] = this.get2DayPercentChange(
                data.untrackedVolumeUSD,
                oneDayData&&oneDayData.untrackedVolumeUSD || 0,
                twoDayData&&twoDayData.untrackedVolumeUSD || 0
            )

            // calculate percentage changes and daily changes
            const [oneDayTxns, txnChange] = this.get2DayPercentChange(
                data.txCount,
                oneDayData&&oneDayData.txCount || 0,
                twoDayData&&twoDayData.txCount || 0
            )

            const priceChangeUSD = this.getPercentChange(
                data&&data.derivedETH * ethPrice,
                parseFloat(oneDayData&&oneDayData.derivedETH || 0) * ethPriceOld
            )

            const currentLiquidityUSD = data&&data.totalLiquidity * ethPrice * data&&data.derivedETH
            const oldLiquidityUSD = oneDayData&&oneDayData.totalLiquidity * ethPriceOld * oneDayData&&oneDayData.derivedETH

            // set data
            data.priceUSD = data&&data.derivedETH * ethPrice
            ...
            const liquidityChangeUSD = this.getPercentChange(currentLiquidityUSD || 0, oldLiquidityUSD || 0)
  ...
  `;
            ...
            return new Promise(function (resolve, reject) {
                req.once(Laya.Event.COMPLETE, this, (result) => {
                   let price= result&&result.data&&result.data.bundles[0]&&result.data.bundles[0].ethPrice
                    resolve(price)
                });
                req.once(Laya.Event.ERROR, this, (data) => {
                });
    ...
    exports.swapUrl=LayaBlock.exUrl;
